...
class ChatViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //get the data from the chat view contoller by triggering the delegate methods
        tableView.dataSource = self
        
        //have to register tableview
        tableView.register(UINib(nibName: K.cellNibName, bundle: nil),
                           forCellReuseIdentifier: K.cellIdentifier)
        
        
        //give naviagtion bar a title
        title = K.appTitle
        
        //hide the back button becuase user has signed in already and nothing to go back to
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        ...
        if let messageBody = messageTextfield.text, let messageSender = Auth.auth().currentUser?.email {
            ...
            db.collection(K.FStore.collectionName).addDocument(data: [K.FStore.senderField: messageSender,
                ...
                if let e = error {
                    print ("There was a proble saving to the database: \(e)")
                }
                else {
